...
class BatchMigrationEnv(gym.Env):
    ...
    def __init__(self, env_parameters):
        ...
        if self.is_full_action:
            ...
            self._action_spec = spaces.Discrete(6)#TODO
            self._action_dim = 6
        ...
        self._num_traces = 1 # env_parameters.num_traces,改为单个，非batch
        ...
        # 改静态变量了 self.users_traces = self._read_traces_from_the_csv(env_parameters.traces_file_path, env_parameters.trace_start_index, self._num_traces)        # 读取一部分用户的轨迹，稀疏化
        self._current_time_slot = 0    # 改非batch
        # 用不到，死代码 self.batch_size = self._num_traces        # 每个批次应该包含的轨迹（或者例子）数量与轨迹数相同
    ...
    # 改为静态函数，在初始化类的时候就执行
    def _read_traces_from_the_csv(file_path, start_index, num_of_traces, trace_length, trace_interval):
        ...
        for line in lines:
            ...
            if items[0] not in user_traces.keys():
                user_traces[items[0]] = []
                user_traces[items[0]].append(Point(x, y))
            else:
                user_traces[items[0]].append(Point(x, y))

        f.close()
        user_names = list(user_traces.keys())
        users_traces_list = []
        for i in range(start_index, (start_index+num_of_traces)):
            user_name = user_names[i]
            one_user_trace = user_traces[user_name][:: trace_interval]         # 应用一个间隔来稀疏化（或降采样）这些数据
            # 这是Python中的切片操作，用于从列表中按一定间隔提取元素。
            # :: 表示从列表的开始到结束。
            # self.trace_interval 是一个整数，表示间隔大小。
            one_user_trace = one_user_trace[0:trace_length]
            users_traces_list.append(one_user_trace)

        return users_traces_list
        
    # 初始化静态变量，python不允许先使用后定义，所以挪到这里了
    users_traces = _read_traces_from_the_csv('./environment/san_traces_coordinate.txt',0 ,150, 100, 3)
    #   函数原型   _read_traces_from_the_csv(file_path, start_index, num_of_traces, trace_length, trace_interval):
    ...
    def _make_state_according_to_action(self, trace_id, action):
        ...
        #                        这个trace_id不用管，没问题，它是整个trace list里的id，不是batch里的
        user_position = users_traces[trace_id][self._current_time_slot]     # _current_time_slot改成非batch了
    ...
    def reset_trace(self, trace_id):
        ...
        state, observation = self._make_state_according_to_action(trace_id, action=None)    # action=None意味着reset操作
    ...
    def reset(self):
        ...
        self.trace_index = random.randint(0, len(self.users_traces))

        state, observation = self.reset_trace(self.trace_index)

        # the true state space dimension is
        self._state = np.array(state, dtype=np.float32)
        self._observation = np.array(observation, dtype=np.float32)
